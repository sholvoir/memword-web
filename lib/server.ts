...
export const postTasks = (tasks: Array<ITask>) =>
	fetch(`${API_URL}/api/v1task`, requestInit(tasks, "POST", authHead));

export const deleteTask = (words: Array<string>) =>
	fetch(`${API_URL}/api/v1task`, requestInit(words, "DELETE", authHead));

export const postIssue = (issue: string) =>
	fetch(`${API_URL}/api/v1issue`, requestInit({ issue }, "POST", authHead));
...
export const postSetting = (setting: ISetting) =>
	fetch(`${API_URL}/api/v1setting`, requestInit(setting, "POST", authHead));
